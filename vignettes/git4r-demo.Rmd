...
```
> library(git4r)
Using global (system default) config: johnhobbs <johnxhobbs@gmail.com>
```
```
...
```
> git_add()
test-git-project-6aee992cf2 directory is not inside a git repository - check this is the top level else ESCAPE and setwd()
Should this directory be turned into a git repo? (Y/N) Y

Copying default .gitignore to working directory
test-git-project-6aee992cf2 is now a git repo 
...
```
<img style="max-width: 100%" src="git_diff_1.png" alt="git_diff(path='.', n=2, n=1)" title="image Title" />
```
...
> git_diff(path='F')
```
<img style="max-width: 100%" src="git_diff_2.png" alt="git_diff(path='F')" title="image Title" />
...
```
<img style="max-width: 100%" src="git_diff_3.png" alt="git_diff('',branch='test-branch', branch='master')" title="image Title" />
```
> git_merge()
...
Creating bare repo at /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2
Adding origin at /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2
```
```
> git_push()
Current branch is not yet pushed to origin -- do this manually
Available remotes
1    origin  /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2
...
New or existing branch name to move to (or hit ENTER to list branches) private-branch
```
```
...
test-git-project-6aee992cf2
```
```
> another_tempdir = tempfile()
> dir.create(another_tempdir)
> git_clone('test-git-project-6aee992cf2', to=another_tempdir)
Cloning  test-git-project-6aee992cf2  into " /tmp/file6ae387d1d8a/test-git-project-6aee992cf2 "
 -- proceed? (Y/N) Y

Cloning to /tmp/file6ae387d1d8a/test-git-project-6aee992cf2
...
[66d1db] (origin @ /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2) dev-branch
(origin @ /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2) HEAD
[a92c7d] (origin @ /tmp/test-default-remote-6ae5bde9f0/test-git-project-6aee992cf2) master
New or existing branch name to move to (or hit ENTER to cancel) 
```
```
git_diff('F', branch='master', branch='origin/dev-branch')
```
<img style="max-width: 100%" src="git_diff_4.png" alt="git_diff('F', branch='master', branch='origin/dev-branch')" title="image Title" />
```
> git_merge()
